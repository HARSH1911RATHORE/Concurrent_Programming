...
void global_init_b( )
{
    threads = malloc( thread_count * sizeof( pthread_t ));
    args = malloc( thread_count * sizeof( size_t ));
    thread_array_bucket = malloc( thread_count * sizeof( FJ_Merge ));
...
void* mergesort_thread( void *args )
{
 ...
 //   printArray( tid.thread_array, right - left +1);
        int i; 
...
////~    right = tid.high_array_index_m;
    ...
    int j;
    for ( int i = tid.low_array_index_b; i <= tid.high_array_index_b; i++ ){
            printf("tid.thread_array_bucket[%d] = %d\n",i,tid.thread_array_bucket[i]);
...
////~			bucket[j]=((tid.thread_array_bucket)[i]);
////~			pthread_mutex_unlock(&lock1);
    ...
    {
        printf("  count value[%d]=%d   ",z,count[z]);
    }
...
/////~        mergeSort( numbers, 0, idx - 1 );         // mergesort is done on unsorted list
		...
		// 	struct task *tsk = &thread_array_merge[i];
		// 	merge(tsk->list, tskm->task_low, tsk->task_low - 1, tsk->task_high);
		// }
    ////~    FJ_Merge tasklist[thread_count];
        FJ_Merge *tskm = &thread_array_merge[0];
...
////~        mergeSort( final_array, 0, idx -1 );
////~        merge( final_array, 0, idx/2, idx -1 );
        /////~
        FILE *ptri;
    ...
    
    {
        ...
        int m,i;
 ////~       int low=0;
     bucket=calloc(idx,sizeof * bucket);
     count=calloc(thread_count,sizeof*count);
 ...
 ////~               thread_array_bucket[ i ].array_length = idx - m;
            ...
            if ( ret )
            {
                ...
                thread_array_bucket[ 0 ].thread_number_bucket = 1;
                                        ...
                                        thread_array_bucket[ i ].low_array_index_b = 0;
                thread_array_bucket[ i ].high_array_index_b = individual_thread_array_length -1;
                bucket_sort_thread( &thread_array_bucket[ 0 ] );
                        for (int b=0;b<thread_count;b++)
        {
             printf("count[%d]=%d",b,count[b]);
        }
        for ( size_t i = 1; i < thread_count; i++ )
        {
            ret = pthread_join( threads[ i ], NULL );
